...
class VBoxContainer(Control):
	def __init__(self, game):
		super().__init__(game)
		self.children: list = []
		self.mesh = QuadMesh(self) # todo temporary?
		self.texture_id: int = 6
	...
	def resize(self):
		self.size = vec2(0, 0)
		greatest_x_size: float = 0.0
		
		for i in range(len(self.children)):
			#self.children[i].parent = self
			self.size.y += self.children[i].size.y
			
			self.children[i].position.y = self.size.y
			self.children[i].position.x = self.position.x
			
			# Scale the container to the widest element.
			if self.children[i].size.x > greatest_x_size:
				greatest_x_size = self.children[i].size.x
		
		self.size.x = greatest_x_size
		self.mesh.rebuild()
		
		for i in range(len(self.children)):
			self.children[i].container_resize()
